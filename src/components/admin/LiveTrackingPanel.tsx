...
import {
  Activity, MousePointerClick, Eye, LogIn, LogOut, Send,
  Server, Database, ShieldCheck, Filter, Pause, Play,
  Loader2, Search, Users, Sparkles, X, Globe, Layers,
} from "lucide-react";
...
import {
  adminActivityOverview, adminTopButtons, adminTopPages, adminTopModules,
  adminActivityTimeseries, adminUserActivity, adminActivityFeed,
...
export function LiveTrackingPanel() {
  ...
  const stats = [
    ...
    { l: "Users (24h)", v: o.unique_users_24h ?? 0, sub: "unique", i: Users, c: "var(--neon-purple)" },
    { l: "Total events", v: o.total_events ?? 0, sub: range.label.toLowerCase(), i: Sparkles, c: "var(--neon-blue)" },
    { l: "Clicks", v: o.total_clicks ?? 0, sub: "button clicks", i: MousePointerClick, c: "#f97316" },
    ...
    <div className="space-y-4">
      ...
      <section className="glass shadow-card-soft relative overflow-hidden rounded-3xl p-5">
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-3">
          <div className="space-y-1.5">
            <Badge className="border-0 bg-emerald-500/20 text-emerald-300"><Activity className="mr-1 h-3 w-3 animate-pulse" /> Live Tracking</Badge>
            <h2 className="font-display text-2xl font-bold tracking-tight md:text-3xl">
              Full-Site <span className="text-gradient">Activity Monitor</span>
            </h2>
            <p className="max-w-2xl text-xs text-muted-foreground">
              Real-time stream of every page view, click, form submit, login, CRUD and admin action across the platform.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button size="sm" variant="outline" className="border-white/15"
              ...
              data-track="tracking.refresh" data-track-module="admin/analytics">
              {overview.isFetching ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />} Refresh
            </Button>
          </div>
        </div>
      </section>
        ...
        {stats.map((s) => (
          <div key={s.l} className="glass shadow-card-soft relative overflow-hidden rounded-2xl p-4">
            ...
            <div className="flex items-center justify-between">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/10 bg-background/40" style={{ boxShadow: `0 0 12px ${s.c}55` }}>
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        <section className="glass shadow-card-soft rounded-2xl p-4 lg:col-span-2">
          <div className="mb-3 flex items-center justify-between">
            <div>
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Events over time</p>
              <p className="font-display text-base font-bold">{range.label} · bucket {range.bucket >= 60 ? `${range.bucket / 60}h` : `${range.bucket}m`}</p>
            </div>
          </div>
          <TimeseriesChart data={timeseries.data ?? []} />
        </section>
        <section className="glass shadow-card-soft rounded-2xl p-4">
          <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Event breakdown</p>
      ...
      <section className="glass shadow-card-soft flex flex-wrap items-center gap-2 rounded-2xl p-3">
        ...
        <div className="relative">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search label, page, id…" className="h-9 w-60 pl-8" />
        </div>
        <Button size="sm" variant={paused ? "default" : "outline"} className="ml-auto" onClick={() => setPaused((p) => !p)}
      ...
      <section className="glass shadow-card-soft rounded-2xl">
        <div className="flex items-center justify-between border-b border-border/40 px-4 py-3">
          <div>
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Real-time activity</p>
            <p className="font-display text-base font-bold">Live feed {paused && <span className="text-amber-400">· paused</span>}</p>
          </div>
          <Badge variant="outline" className="text-[10px]">{combinedFeed.length} events</Badge>
        </div>
        <div className="max-h-[520px] overflow-y-auto font-mono text-[11px]">
          {feed.isLoading && combinedFeed.length === 0 ? (
            ...
            <div className="flex h-40 items-center justify-center text-muted-foreground">No events match the filters.</div>
            ...
            return (
              ...
              >
                <span className="w-20 shrink-0 text-muted-foreground">{fmtTime(row.created_at)}</span>
                <span className="flex w-32 shrink-0 items-center gap-1.5 rounded-md border px-1.5 py-0.5 text-[10px]" style={{ borderColor: `${color}66`, color }}>
                ...
                <span className="flex-1 truncate text-muted-foreground">
                  <span className="text-foreground/80">{row.element_label ?? row.target_kind ?? row.event_type}</span>
                  {row.page_path && <span className="ml-2 text-muted-foreground">{row.page_path}</span>}
                </span>
                {row._live && <Badge className="ml-2 border-0 bg-emerald-500/20 text-[10px] text-emerald-300">new</Badge>}
              </button>
            );
          })}
        </div>
          ...
          {rows.map((r, i) => (
            <li key={`${r.primary}-${i}`} className="space-y-1">
              <div className="flex items-center justify-between gap-2 text-xs">
                ...
                <span className="shrink-0 font-mono text-foreground/80">{r.value.toLocaleString()}</span>
              </div>
              <div className="h-1.5 overflow-hidden rounded-full bg-background/40">
                <div className="h-full rounded-full bg-cta-gradient" style={{ width: `${(r.value / max) * 100}%` }} />
              </div>
...
function TimeseriesChart({ data }: { data: Array<{ bucket: string; event_type: string; event_count: number }> }) {
  ...
  const points = values.map((v, i) => `${(i / Math.max(1, values.length - 1)) * 100},${100 - (v / max) * 90}`).join(" ");
  return (
    <>
...
function EventBreakdown({ data }: { data: Array<{ bucket: string; event_type: string; event_count: number }> }) {
  const totals = new Map<string, number>();
  for (const r of data) totals.set(r.event_type, (totals.get(r.event_type) ?? 0) + Number(r.event_count));
  const rows = Array.from(totals.entries()).sort((a, b) => b[1] - a[1]);
  const sum = rows.reduce((s, [, v]) => s + v, 0) || 1;
  if (rows.length === 0) return <p className="py-10 text-center text-xs text-muted-foreground">No events.</p>;
  return (
    ...
    <div className="mt-4 space-y-4">
      <div className="rounded-2xl border border-border/40 bg-background/40 p-4">
        <div className="mb-2 flex items-center gap-2">
          <span className="flex items-center gap-1.5 rounded-md border px-2 py-0.5 text-[11px]" style={{ borderColor: `${color}66`, color }}>
            <Icon className="h-3 w-3" /> {row.event_type}
          </span>
          <span className="text-xs text-muted-foreground">{timeAgo(row.created_at)} · {fmtTime(row.created_at)}</span>
        </div>
        <p className="font-display text-base font-bold">{row.element_label ?? row.target_kind ?? row.event_type}</p>
        <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-[11px]">
          <dt className="text-muted-foreground">User</dt><dd>{row.user_name}</dd>
          <dt className="text-muted-foreground">Page</dt><dd className="truncate">{row.page_path ?? "—"}</dd>
          <dt className="text-muted-foreground">Module</dt><dd>{row.module ?? "—"}</dd>
          <dt className="text-muted-foreground">Element id</dt><dd className="font-mono">{row.element_id ?? "—"}</dd>
          {row.target_kind && (<>
            <dt className="text-muted-foreground">Target</dt>
            <dd className="truncate">{row.target_kind}{row.target_id ? ` · ${row.target_id}` : ""}</dd>
          </>)}
        </dl>
      ...
      <div className="rounded-2xl border border-border/40 bg-background/40 p-4">
        <p className="mb-2 text-[10px] uppercase tracking-wider text-muted-foreground">Recent activity by this user</p>
        {!row.user_id ? (
          <p className="text-xs text-muted-foreground">Anonymous event.</p>
        ) : userQ.isLoading ? (
          <div className="flex items-center text-xs text-muted-foreground"><Loader2 className="mr-2 h-3 w-3 animate-spin" />Loading…</div>
        ) : (userQ.data ?? []).length === 0 ? (
            ...
            {(userQ.data ?? []).map((e) => {
              ...
              return (
                <li key={e.id} className="flex items-center gap-2 truncate">
                  <span className="w-16 shrink-0 text-muted-foreground">{fmtTime(e.created_at)}</span>
                  <span className="w-24 shrink-0 truncate" style={{ color: c }}>{e.event_type}</span>
