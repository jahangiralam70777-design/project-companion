...
import {
  ...
  Users, Target, FileDown, TrendingUp, Activity,
  Layers, FileText, PlayCircle,
  Brain, Lightbulb, AlertTriangle, Trophy, Flame, Award,
  Eye, ListChecks, Loader2, BookOpen, BarChart3,
  Bookmark, GraduationCap, History, Code, Filter, ArrowUp, ArrowDown,
  RefreshCw,
} from "lucide-react";
    ...
    <div className="space-y-4">
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-4">
          <div className="space-y-2">
            ...
            <h1 className="font-display text-3xl font-bold tracking-tight md:text-4xl">
              Analytics &amp; <span className="bg-gradient-to-r from-violet-400 via-fuchsia-400 to-blue-400 bg-clip-text text-transparent">Reports</span>
            </h1>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {q.isFetching ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCw className="mr-2 h-4 w-4" />} Refresh
            </Button>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="rounded-xl"><FileDown className="mr-2 h-4 w-4" /> Export</Button>
              </DropdownMenuTrigger>
      ...
      <section className="glass shadow-card-soft flex flex-wrap items-center gap-2 rounded-2xl p-3">
        <Select value={String(rangeIdx)} onValueChange={(v) => setRangeIdx(Number(v))}>
          <SelectTrigger className="h-9 w-44 rounded-xl"><Calendar className="mr-1 h-3.5 w-3.5" /><SelectValue /></SelectTrigger>
          <SelectContent>
            {RANGES.map((r, i) => <SelectItem key={r.l} value={String(i)}>{r.l}</SelectItem>)}
          </SelectContent>
        </Select>
        <Select value={levelFilter} onValueChange={setLevelFilter}>
          <SelectTrigger className="h-9 w-44 rounded-xl"><GraduationCap className="mr-1 h-3.5 w-3.5" /><SelectValue placeholder="All Levels" /></SelectTrigger>
          <SelectContent>
        ...
        {q.isFetching && <Badge variant="outline" className="gap-1 rounded-full text-[10px]"><Loader2 className="h-3 w-3 animate-spin" /> updating</Badge>}
        {error && <Badge variant="outline" className="gap-1 rounded-full border-red-500/40 text-[10px] text-red-400"><AlertTriangle className="h-3 w-3" /> {error.message}</Badge>}
      ...
      {loading ? (
        ...
        <section className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
          {[
            { l: "Total Students", v: data?.kpis.totalUsers ?? 0, i: Users, tone: "from-violet-500/30 to-fuchsia-500/20 text-violet-400", trend: 18.5, to: "/admin/users" as const },
            { l: "Active Students", v: data?.kpis.activeUsers ?? 0, i: Activity, tone: "from-emerald-500/30 to-cyan-500/20 text-emerald-400", trend: 16.3, to: "/admin/users" as const },
            { l: "Daily Active", v: data?.kpis.dau ?? 0, i: Flame, tone: "from-orange-500/30 to-amber-500/20 text-orange-400", trend: 12.4, to: "/admin/users" as const },
            { l: "MCQs Solved", v: data?.kpis.mcqsSolved ?? 0, i: ListChecks, tone: "from-blue-500/30 to-indigo-500/20 text-blue-400", trend: 24.7, to: "/admin/mcq" as const },
            { l: "Mock Attempts", v: data?.kpis.mockAttempts ?? 0, i: Trophy, tone: "from-amber-500/30 to-orange-500/20 text-amber-400", trend: 18.2, to: "/admin/mock-test" as const },
            { l: "Quiz Attempts", v: data?.kpis.quizAttempts ?? 0, i: Target, tone: "from-cyan-500/30 to-teal-500/20 text-cyan-400", trend: 20.1, to: "/admin/quiz" as const },
          ].map((s) => (
            <button
              key={s.l}
              type="button"
            ...
            >
              ...
              <div className={`mb-3 inline-flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br ${s.tone}`}>
              ...
              <p className="mt-1 flex items-center gap-1 text-[11px] font-medium text-emerald-400">
                ...
                <span className="text-muted-foreground">vs last {RANGES[rangeIdx].l.split(" ").slice(-2).join(" ")}</span>
              </p>
            </button>
          ))}
        </section>
      ...
      {!loading && (
        <section className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
          {[
            { l: "WAU", v: data?.kpis.wau ?? 0, sub: "Weekly Active Users", trend: data?.kpis.wauGrowth ?? 0, c: "text-emerald-400" },
            { l: "MAU", v: data?.kpis.mau ?? 0, sub: "Monthly Active Users", trend: data?.kpis.mauGrowth ?? 0, c: "text-sky-400" },
            { l: "New Sign-ups", v: data?.kpis.newUsers ?? 0, sub: "vs last 30 days", trend: 21.3, c: "text-fuchsia-400" },
            { l: "Avg Accuracy", v: `${data?.kpis.accuracy ?? 0}%`, sub: "Completed attempts", trend: 8.5, c: "text-amber-400" },
            { l: "Engagement", v: formatDuration(data?.kpis.avgEngagementSec ?? 0), sub: "Avg. per learner", trend: 15.3, c: "text-violet-400" },
            { l: "Downloads", v: data?.kpis.downloads ?? 0, sub: "Notes + Q-bank", trend: 19.4, c: "text-cyan-400" },
          ].map((k) => (
            <div key={k.l} className="glass shadow-card-soft rounded-2xl p-3">
              <div className="flex items-start justify-between">
                ...
                <span className={`flex items-center gap-0.5 text-[10px] font-medium ${k.trend >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
                  {k.trend >= 0 ? <ArrowUp className="h-2.5 w-2.5" /> : <ArrowDown className="h-2.5 w-2.5" />}
