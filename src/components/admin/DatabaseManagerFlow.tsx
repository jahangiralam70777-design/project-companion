...
import {
  Activity, Database, Download, HardDrive, RefreshCw, ShieldAlert, ShieldCheck,
  TrendingUp, Users, ListChecks, Layers, Server, Clock, Eye, Search, Trash2,
  Loader2, Radio, Plus, Pencil, Play, X, Network, FileJson, FileSpreadsheet,
...
import {
  adminListTableRows, adminDeleteTableRow, adminBulkDeleteTableRows,
  adminUpsertTableRow, adminListPublicTables, adminGetTableMetadata,
  adminRunSelectQuery, adminGlobalSearch, PROTECTED_WRITE_TABLES,
  type TableMetadata,
...
function formatNumber(n: number): string { return new Intl.NumberFormat("en").format(n ?? 0); }

function KpiCard({ icon: Icon, label, value, hint, accent = "from-[var(--neon-purple)] to-[var(--neon-blue)]" }: {
  icon: typeof Database; label: string; value: string | number; hint?: string; accent?: string;
}) {
  return (
    ...
    <div className="space-y-5">
      <div className="glass-card flex flex-col gap-3 rounded-3xl p-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button size="sm" onClick={() => { downloadBlob(toCsv(data), "text/csv;charset=utf-8", `database-report-${new Date().toISOString().slice(0, 10)}.csv`); toast.success("Report downloaded"); }}>
...
function OverviewTab({ data, usagePct }: { data: DatabaseManagerStats; usagePct: number }) {
  ...
  return (
    <>
      ...
      <SectionCard title="Storage usage" action={<Badge variant="outline" className="text-xs">{formatBytes(data.storage.dbSizeBytes)} / {formatBytes(data.storage.capacityBytes)}</Badge>}>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <SectionCard title="Storage by table" action={<Badge variant="outline" className="text-xs">Top {otherTablesShown.length}</Badge>}>
          {otherTablesShown.length === 0 ? <p className="text-sm text-muted-foreground">No data available</p> : (
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <SectionCard title="MCQs by subject">
          {data.mcqBySubject.length === 0 ? <p className="text-sm text-muted-foreground">No data available</p> : (
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
        ...
        <SectionCard title="Top chapters by MCQ count">
          {data.mcqByChapter.length === 0 ? <p className="text-sm text-muted-foreground">No data available</p> : (
            <div className="max-h-64 overflow-y-auto">
              <table className="w-full text-sm">
                <thead className="text-xs uppercase text-muted-foreground">
                  <tr><th className="py-2 text-left">Chapter</th><th className="py-2 text-left">Subject</th><th className="py-2 text-right">MCQs</th></tr>
                </thead>
                <tbody>
...
function TablesTab() {
  ...
  return (
    <SectionCard
      ...
      action={
        <div className="flex items-center gap-2">
          <Input value={filter} onChange={(e) => setFilter(e.target.value)} placeholder="Filter tables…" className="h-8 w-44" />
    ...
    >
      {isLoading ? (
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 9 }).map((_, i) => <Skeleton key={i} className="h-14 rounded-xl" />)}
        </div>
      ) : filtered.length === 0 ? (
            ...
            >
              <div className="min-w-0">
                <div className="flex items-center gap-1.5">
                  ...
                  {PROTECTED_WRITE_TABLES.has(t.table_name) && <Lock className="h-3 w-3 text-amber-400" />}
                </div>
                <p className="mt-0.5 text-[11px] text-muted-foreground">
                  ~{formatNumber(t.row_estimate)} rows · {formatBytes(t.size_bytes)} · RLS {t.rls_enabled ? "on" : "off"}
                </p>
              </div>
      ...
      {selected ? <TableDetailDialog table={selected} onClose={() => setSelected(null)} onSwitch={setSelected} /> : null}
    </SectionCard>
  );
}
...
function formatCell(value: unknown): string {
  ...
  try { const s = JSON.stringify(value); return s.length > 80 ? s.slice(0, 77) + "…" : s; }
  catch { return String(value); }
}

function TableDetailDialog({ table, onClose, onSwitch }: { table: string; onClose: () => void; onSwitch: (t: string) => void }) {
  const qc = useQueryClient();
  ...
  useEffect(() => {
    setPage(0); setSearch(""); setSearchInput(""); setSortColumn(undefined);
    setSortDir("desc"); setSelectedIds(new Set()); setEditor(null);
    setDeleteId(null); setConfirmStage(0); setBulkConfirm(false);
  }, [table]);

  const rowsQuery = useQuery({
    queryKey: ["admin-table-rows", table, page, search, sortColumn, sortDir] as const,
    queryFn: () => adminListTableRows({ data: { table, page, pageSize, search: search || undefined, sortColumn, sortDir } }),
  ...
  useEffect(() => {
    const channel = supabase.channel(`db-mgr-${table}`)
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setDeleteId(null); setConfirmStage(0);
    },
  ...
  const bulkDeleteMutation = useMutation({
    ...
    onError: (e: Error) => { toast.error(e.message); setBulkConfirm(false); },
  });
  ...
  const visibleIds = visibleRows.map((r) => String((r as Record<string, unknown>).id ?? "")).filter(Boolean);
  ...
  function exportRows(format: "csv" | "json") {
    ...
    if (format === "json") {
      downloadBlob(JSON.stringify(rowsQuery.data.rows, null, 2), "application/json", `${table}-page${page + 1}-${stamp}.json`);
    } else {
      downloadBlob(rowsToCsv(columns, rowsQuery.data.rows as Record<string, unknown>[]), "text/csv;charset=utf-8", `${table}-page${page + 1}-${stamp}.csv`);
    }
  ...
  return (
    <>
        ...
        <DialogContent className="max-w-6xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 font-mono">
              ...
              {isProtected && <Badge variant="outline" className="gap-1 border-amber-500/30 bg-amber-500/10 text-amber-300 text-[10px]"><Lock className="h-3 w-3" />Protected</Badge>}
            </DialogTitle>
            <DialogDescription>
              Live data · {rowsQuery.data ? `${formatNumber(rowsQuery.data.total)} rows total` : "loading…"} · {metaQuery.data?.columns.length ?? "—"} columns
            </DialogDescription>
          </DialogHeader>
          ...
          <Tabs defaultValue="rows">
            ...
            <TabsContent value="rows" className="space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Button size="sm" variant="outline" onClick={() => { setSearch(searchInput); setPage(0); }}>Search</Button>
                <Button size="sm" variant="outline" onClick={() => rowsQuery.refetch()} disabled={rowsQuery.isFetching}>
                  <RefreshCw className={`mr-2 h-4 w-4 ${rowsQuery.isFetching ? "animate-spin" : ""}`} /> Refresh
                </Button>
                {!isProtected && (
                ...
                <Button size="sm" variant="outline" onClick={() => exportRows("csv")} disabled={visibleRows.length === 0}>
                  <FileSpreadsheet className="mr-2 h-4 w-4" /> CSV
                </Button>
                <Button size="sm" variant="outline" onClick={() => exportRows("json")} disabled={visibleRows.length === 0}>
              ...
              <div className="max-h-[55vh] overflow-auto rounded-lg border border-border/60">
                {rowsQuery.error ? (
                  <div className="p-4 text-sm text-rose-300">{(rowsQuery.error as Error).message}</div>
                ) : !rowsQuery.data ? (
                  <div className="space-y-2 p-3">{Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-6 w-full" />)}</div>
                  ...
                  <table className="w-full text-xs">
                          ...
                          <th key={c} className="whitespace-nowrap border-b border-border/60 px-2 py-2 text-left font-mono text-[10px] uppercase text-muted-foreground">
                            <button
                              ...
                              onClick={() => {
                                ...
                                else { setSortColumn(c); setSortDir("asc"); }
                              }}
                            >
                    ...
                    <tbody>
                      {visibleRows.map((row, idx) => {
                        ...
                        return (
                          <tr key={rid || idx} className="border-b border-border/40 hover:bg-card/40">
                            {!isProtected && (
                              <td className="px-2 py-1.5">
                                {rid ? (
                                  <Checkbox
                                    checked={selectedIds.has(rid)}
                                    onCheckedChange={(v) => setSelectedIds((prev) => { const n = new Set(prev); if (v) n.add(rid); else n.delete(rid); return n; })}
                                  />
                                ) : null}
                              </td>
                            )}
                            ...
                            <td className="sticky right-0 bg-card/95 px-2 py-1.5">
                              <div className="flex items-center justify-end gap-1">
                                {!isProtected && rid ? (
                                  <>
                                    <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => setEditor({ mode: "edit", row: row as Record<string, unknown> })}>
                                      <Pencil className="h-3.5 w-3.5" />
                                    </Button>
                                    <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-rose-400 hover:bg-rose-500/10"
                                      onClick={() => { setDeleteId(rid); setConfirmStage(1); }}>
              ...
              <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
                <span>Page {page + 1} of {totalPages} · {selectedIds.size} selected</span>
                <div className="flex items-center gap-2">
                  <Button size="sm" variant="outline" disabled={page === 0 || rowsQuery.isFetching} onClick={() => setPage((p) => Math.max(0, p - 1))}>Prev</Button>
                  <Button size="sm" variant="outline" disabled={page + 1 >= totalPages || rowsQuery.isFetching} onClick={() => setPage((p) => p + 1)}>Next</Button>
                </div>
              </div>
            </TabsContent>
            ...
            <TabsContent value="rels">
              <RelationshipsPanel meta={metaQuery.data} loading={metaQuery.isLoading} onOpen={onSwitch} />
            </TabsContent>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(o) => { if (!o) { setDeleteId(null); setConfirmStage(0); } }}>
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            ...
            {confirmStage === 1 ? (
              <AlertDialogAction onClick={(e) => { e.preventDefault(); setConfirmStage(2); }} className="bg-rose-500 text-white hover:bg-rose-600">
                Continue
              </AlertDialogAction>
            ) : (
              <AlertDialogAction
      ...
      <AlertDialog open={bulkConfirm} onOpenChange={setBulkConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This permanently deletes the selected rows from "{table}". This cannot be undone.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={bulkDeleteMutation.isPending}>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={(e) => { e.preventDefault(); bulkDeleteMutation.mutate(Array.from(selectedIds)); }}
...
function SchemaPanel({ meta, loading }: { meta?: TableMetadata; loading: boolean }) {
  if (loading || !meta) return <div className="space-y-2 p-3">{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-6 w-full" />)}</div>;
  return (
    <div className="max-h-[55vh] overflow-auto rounded-lg border border-border/60">
      <table className="w-full text-xs">
...
function RelationshipsPanel({ meta, loading, onOpen }: { meta?: TableMetadata; loading: boolean; onOpen: (t: string) => void }) {
  if (loading || !meta) return <Skeleton className="h-32 w-full" />;
  return (
    <div className="space-y-4 max-h-[55vh] overflow-auto">
      <div>
        <h4 className="mb-2 flex items-center gap-2 text-sm font-semibold"><Network className="h-4 w-4" /> References (this → other)</h4>
        {meta.foreign_keys.length === 0 ? <p className="text-xs text-muted-foreground">No outgoing foreign keys.</p> : (
          <ul className="space-y-1 text-xs font-mono">
            {meta.foreign_keys.map((f) => (
              <li key={f.constraint_name} className="flex items-center justify-between rounded-md border border-border/60 px-2 py-1.5">
                <span>{f.columns.join(", ")} → <button className="text-primary underline-offset-2 hover:underline" onClick={() => onOpen(f.foreign_table)}>{f.foreign_table}</button>({f.foreign_columns.join(", ")})</span>
      ...
      <div>
        <h4 className="mb-2 flex items-center gap-2 text-sm font-semibold"><Network className="h-4 w-4 rotate-180" /> Referenced by (other → this)</h4>
        {meta.referenced_by.length === 0 ? <p className="text-xs text-muted-foreground">No tables reference this one.</p> : (
          <ul className="space-y-1 text-xs font-mono">
            {meta.referenced_by.map((f) => (
              <li key={f.constraint_name} className="flex items-center justify-between rounded-md border border-border/60 px-2 py-1.5">
                <span><button className="text-primary underline-offset-2 hover:underline" onClick={() => onOpen(f.from_table)}>{f.from_table}</button>({f.from_columns.join(", ")}) → {f.columns.join(", ")}</span>
                <span className="text-[10px] text-muted-foreground">{f.constraint_name}</span>
              </li>
            ))}
          </ul>
...
function PoliciesPanel({ meta, loading }: { meta?: TableMetadata; loading: boolean }) {
  if (loading || !meta) return <Skeleton className="h-32 w-full" />;
  if (meta.policies.length === 0) return <p className="text-sm text-muted-foreground">No RLS policies defined on this table.</p>;
  return (
      ...
      {meta.policies.map((p) => (
        <div key={p.name} className="rounded-md border border-border/60 p-3 text-xs">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="outline" className="text-[10px]">{p.command} · {p.permissive}</Badge>
          </div>
          <p className="mt-1 text-muted-foreground">Roles: {Array.isArray(p.roles) ? p.roles.join(", ") : "—"}</p>
          {p.using ? <pre className="mt-1 whitespace-pre-wrap break-all font-mono text-[11px]">USING: {p.using}</pre> : null}
          {p.with_check ? <pre className="mt-1 whitespace-pre-wrap break-all font-mono text-[11px]">WITH CHECK: {p.with_check}</pre> : null}
        </div>
      ))}
    </div>
  );
...
function IndexesPanel({ meta, loading }: { meta?: TableMetadata; loading: boolean }) {
  if (loading || !meta) return <Skeleton className="h-32 w-full" />;
  if (meta.indexes.length === 0) return <p className="text-sm text-muted-foreground">No indexes.</p>;
  return (
    <ul className="space-y-1 max-h-[55vh] overflow-auto text-xs font-mono">
      {meta.indexes.map((i) => (
        <li key={i.name} className="rounded-md border border-border/60 p-2">
          ...
          <pre className="mt-1 whitespace-pre-wrap break-all text-[11px] text-muted-foreground">{i.definition}</pre>
        </li>
      ))}
    </ul>
  );
...
function RowEditorDialog({
  table, meta, mode, row, onClose, onSaved,
}: {
  table: string; meta?: TableMetadata; mode: "create" | "edit";
  row?: Record<string, unknown>; onClose: () => void; onSaved: () => void;
}) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      for (const col of meta.columns) {
        ...
        if (raw === "" && col.is_nullable) { payload[col.name] = null; continue; }
        ...
        if (t === "boolean") payload[col.name] = raw === "true" || raw === "1";
          ...
          const n = Number(raw); if (!Number.isFinite(n)) throw new Error(`Invalid number for ${col.name}`);
          payload[col.name] = n;
        } else if (t === "jsonb" || t === "json") {
          try { payload[col.name] = JSON.parse(raw); } catch { throw new Error(`Invalid JSON for ${col.name}`); }
        } else {
    ...
    onSuccess: () => { toast.success(mode === "create" ? "Row created" : "Row updated"); onSaved(); },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return (
    <Dialog open onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{mode === "create" ? "Create row" : "Edit row"} · <span className="font-mono">{table}</span></DialogTitle>
          <DialogDescription>{mode === "edit" ? `Editing ${pk} = ${String((row as Record<string, unknown>)[pk])}` : "Leave a field blank to use its default."}</DialogDescription>
        </DialogHeader>
            ...
            return (
              <div key={c.name} className="space-y-1">
                <label className="text-xs font-medium">
                  ...
                  <span className="ml-2 text-muted-foreground">{c.data_type}{c.is_nullable ? " · nullable" : ""}</span>
                </label>
                {isJson ? (
                  <Textarea rows={3} value={values[c.name] ?? ""} onChange={(e) => setValues((v) => ({ ...v, [c.name]: e.target.value }))} placeholder='e.g. {"key": "value"}' />
                ) : c.data_type === "boolean" ? (
                  <select className="h-9 w-full rounded-md border border-input bg-background px-2 text-sm" value={values[c.name] ?? ""} onChange={(e) => setValues((v) => ({ ...v, [c.name]: e.target.value }))}>
                    <option value="">(default)</option><option value="true">true</option><option value="false">false</option>
                  </select>
                ) : (
                  <Input value={values[c.name] ?? ""} onChange={(e) => setValues((v) => ({ ...v, [c.name]: e.target.value }))} placeholder={c.default ?? ""} />
                )}
              </div>
            );
          })}
...
function SearchTab() {
  ...
  return (
    <SectionCard title="Global database search" action={<Badge variant="outline" className="text-xs">Searches name / title / email / slug / question / description across all tables</Badge>}>
      <div className="flex gap-2">
        <Input value={term} onChange={(e) => setTerm(e.target.value)} placeholder="Type at least 2 characters…" onKeyDown={(e) => { if (e.key === "Enter") setSubmitted(term); }} />
        <Button onClick={() => setSubmitted(term)} disabled={term.length < 2}><Search className="mr-2 h-4 w-4" /> Search</Button>
      </div>
      <div className="mt-4">
        {!submitted ? <p className="text-sm text-muted-foreground">Enter a search term to find matching records across all tables.</p>
          : query.isLoading ? <Skeleton className="h-32 w-full" />
          : query.error ? <p className="text-sm text-rose-300">{(query.error as Error).message}</p>
          : !query.data || query.data.length === 0 ? <p className="text-sm text-muted-foreground">No matches.</p>
          : (
            <div className="max-h-[55vh] overflow-auto rounded-lg border border-border/60">
              <table className="w-full text-xs">
                <thead className="sticky top-0 bg-card/95"><tr className="text-muted-foreground">
                  <th className="px-2 py-2 text-left">Table</th><th className="px-2 py-2 text-left">ID</th><th className="px-2 py-2 text-left">Match</th>
                </tr></thead>
                <tbody>
                  {query.data.map((r, i) => (
...
function QueryConsoleTab() {
  const [sql, setSql] = useState("SELECT id, display_name, status, last_login_at FROM profiles ORDER BY last_login_at DESC NULLS LAST LIMIT 25");
  ...
  return (
    <div className="space-y-4">
      <SectionCard title="Query Console" action={<Badge variant="outline" className="text-xs">SELECT / WITH only · max 500 rows</Badge>}>
        <Textarea rows={5} value={sql} onChange={(e) => setSql(e.target.value)} className="font-mono text-xs" />
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <Button onClick={() => mutation.mutate()} disabled={mutation.isPending || sql.trim().length === 0}>
            {mutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Play className="mr-2 h-4 w-4" />}
            Run query
          </Button>
          {rows.length > 0 && (
            <>
              <Button size="sm" variant="outline" onClick={() => downloadBlob(rowsToCsv(columns, rows), "text/csv;charset=utf-8", `query-${Date.now()}.csv`)}>
                <FileSpreadsheet className="mr-2 h-4 w-4" /> CSV
              </Button>
              <Button size="sm" variant="outline" onClick={() => downloadBlob(JSON.stringify(rows, null, 2), "application/json", `query-${Date.now()}.json`)}>
                <FileJson className="mr-2 h-4 w-4" /> JSON
              </Button>
            </>
          )}
          <span className="text-xs text-muted-foreground">{rows.length > 0 ? `${rows.length} rows` : ""}</span>
        </div>
        {mutation.error ? <p className="mt-3 text-sm text-rose-300">{(mutation.error as Error).message}</p> : null}
      </SectionCard>
      ...
      {rows.length > 0 && (
        <SectionCard title="Results">
          <div className="max-h-[55vh] overflow-auto rounded-lg border border-border/60">
            <table className="w-full text-xs">
              <thead className="sticky top-0 bg-card/95"><tr>
                {columns.map((c) => <th key={c} className="whitespace-nowrap border-b border-border/60 px-2 py-2 text-left font-mono text-[10px] uppercase text-muted-foreground">{c}</th>)}
              </tr></thead>
              <tbody>
                {rows.map((r, idx) => (
                  <tr key={idx} className="border-b border-border/40">
                    {columns.map((c) => <td key={c} className="whitespace-nowrap px-2 py-1.5 font-mono">{formatCell(r[c])}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <SectionCard title="History">
          <ul className="space-y-1 text-xs font-mono">
            {historyRef.current.map((q, i) => (
              <li key={i}>
                <button className="w-full truncate rounded-md border border-border/60 px-2 py-1 text-left hover:bg-card/70" onClick={() => setSql(q)}>{q}</button>
              </li>
            ))}
          </ul>
        </SectionCard>
